...
function checkStatus(percentageChange) {
    var sts = ''

    if (percentageChange > 0 || percentageChange == '>100') {
        sts = '<i class="fa fa-chevron-circle-down" aria-hidden="true" style="float:left;position: relative;top: 2px;left: 5px;margin-right: 10px;font-size:20px;color:red;"></i>'
    } else if (percentageChange == '0') {
        sts = ''
    } else {
        sts = '<i class="fa fa-chevron-circle-up" aria-hidden="true" style="float:left;position: relative;top: 2px;left: 5px;margin-right: 10px;font-size:20px;color:green;"></i>'
    }
...
function fillExpenseHeader(object, tid, head) {
    ...
    document.getElementById(tid).innerHTML = `<th style="width:40%">${rowbtn} <a href="${head}/?selected_date=${choosen_month}"  style="text-decoration: none">${head}</a><br><em style="padding-left:30px">Change: ${object.change}</em></th>` +
        '<td style="width: 15%; text-align:right;">' + object.current + '</td>' +
        '<td style="width: 15%; text-align:right;">' + object.previous + '</td>' +
        '<td style="width: 15%; text-align:right;">' + checkStatus(object.per_change) + object.per_change + '%</td>' +
